...
//  else Goal_speed = 2*goal;  //两轮差速限幅
	...
	error = Goal_speed - 0.0017f*10*Left_Encoder;            //V = N/1024*（30/68*20.1/100*20*10） = N*0.0017*10 m/s  30为编码器齿轮，68为传动齿轮，20.1为轮子周长，控制周期50ms
  error_sum += error;
...
{
		pwm_init(PWM_R, 17000, 6000); //初始化PWM1  使用P60引脚  初始化频率为17Khz
		pwm_init(PWM_L, 17000, 6000); //初始化PWM2  使用P62引脚  初始化频率为17Khz
